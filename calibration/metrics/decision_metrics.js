...
function expectedTransitionSign(t,truth){
  ...
  if(Number.isFinite(e)&&a>0&&b>0)return Math.sign(Math.pow(b/a,e)-1);
  const fallback=Number(truth?.trueEffect);return Number.isFinite(fallback)?Math.sign(fallback):0;
}
